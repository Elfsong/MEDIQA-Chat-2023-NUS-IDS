...
def getNextActZSL(input_string, **generator_args):
    input_ids = nextact_tokenizer.encode(input_string, return_tensors="pt").to(device)
    res = nextact_model.generate(input_ids, **generator_args)
    temp = nextact_tokenizer.batch_decode(res, skip_special_tokens=True)[0]
 ...
 #           break
      ...
      #                  print ("pu for qa "+pu)
            ...
            if dact!="":
                source2 +=" [PUZSL] "+dact
            if puzsl!="":
                source2 +=" [PUDACT] "+puzsl ##accidental flipping here 
            next_speaker_act = getNextActZSL(source2) 
            #################
