...
def get_save_dir(base_dir, name):
    ...
    # Increment from the last one
    if len(match_dirs) == 0:  
        save_dir = base_path + "-01"
    else:       
        last = match_dirs[-1]
        r = re.match(".*-(\d\d)", last)
        next_num = int(r.group(1)) + 1

        save_dir = base_path + f'-{next_num:02d}'

    # log.info(f'Will create new save_dir: {save_dir}')
...
def get_logger(log_dir, name, log_level="debug"):
    ...
    # console_formatter = logging.Formatter('[%(asctime)s] %(message)s', datefmt='%m.%d.%y %H:%M:%S')
    console_formatter = logging.Formatter('[%(asctime)s] [%(filename)s:%(lineno)s - %(funcName)s()] %(message)s', datefmt='%m.%d %H:%M:%S')
...
def masked_token_match(tgt_ids: torch.tensor, outputs: torch.tensor, return_indices=False):
    """
...
def visualize(tbx, pred_dict, step, split, num_visuals):
    ...
    for i in range(num_visuals):
        ...
        tbx.add_text(tag=f'{split}/{i+1}_of_{num_visuals}', text_string=tbl_fmt, global_step=step)
