...
def RBgetNextAct(speaker, secname):


    if speaker=="Doctor":
        if secname in ['EXAMINATION','PLAN','ASSESSMENT', 'REVIEW', 'MEDICATION']:
            ch = random.randint(0,9)
            if ch==0:
                return "DIRECTIVE [SEP] "+secname
            elif ch==1:
                return "QUESTION [SEP] "+secname
            else:
...
gen_tokenizerdir="./trained_models/utterance/tokenizer"
gen_dialogmodeldir="./trained_models/utterance/model"
...
ddialog_model_dir="./trained_models/dact/model"
...
def getConversationFromReport(lines):
 ...
 #           break
      ...
      #                  print ("pu for qa "+pu)
            ...
            next_speaker_act = RBgetNextAct(nspeakers[-1], secname)
            #################
...
if __name__=="__main__":
    ...
    inpd = "/nas/gsdas/t5qg/mediQA/finalweek/taskc/train"
    outd = "/nas/gsdas/t5qg/mediQA/finalweek/taskc/out2"


    for fn in os.listdir(inpd):

        if "D2N065" not in fn or not fn.endswith("src"):
        #if not fn.endswith("src"):
            continue

        inpf = inpd+"/"+fn
        outfile = outd+"/"+fn.replace(".src",".txt")
        print ("Processing "+inpf)
        lines=open (inpf, "r").readlines()

        tlines = getConversationFromReport(lines)
        print ("#lines in conversation "+str(len(tlines)))
        fout = open (outfile, "w")
        for tline in tlines:
            fout.write(tline.strip()+"\n")
            fout.flush()
        fout.close()
